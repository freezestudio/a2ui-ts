...
import { A2UIRendererService, A2UISurface, CatalogRegistry, getBasicComponentRegistrations } from '@a2ui/angular';
 ...
 * 由 @a2ui/angular 渲染器 + basic catalog 组件渲染；按钮触发 renderer→agent action。
 */
@Component({
  selector: 'app-root',
  standalone: true,
    ...
    this.renderer.processMessages([
      {
        ...
        createSurface: {
          ...
          components: [
            ...
            { id: 'body', component: 'Text', text: '本界面由 @a2ui/angular 渲染器驱动，组件由 A2UI 消息声明。' },
            {
              id: 'btn',
              component: 'Button',
              child: 'btn-label',
