# a2ui-ts 实施路线图（源自 geo-scout docs/a2ui-split-plan.md，审核通过）

> 状态：**已审核通过，实施中**（原计划文档：geo-scout docs/a2ui-split-plan.md）
...
- **官方仓库是现成范本**：yarn workspace 单仓、`renderers/{web_core,angular,...}`、`agent_sdks/python`、`specification/{v0_8..v1_0}/test+eval`、`samples/`、`tools/`、`conformance/`，命名 `@a2ui/*`
...
| 资产 | 位置 | 规模 | 独立性 | 去向 |
|---|---|---|---|---|
| a2ui-shared | `packages/a2ui/shared` | 1.3K 行 | ✅ 零依赖，已下沉工具 | → `@a2ui/shared` |
| a2ui-web-core | `packages/a2ui/web-core` | 2.1K 行 | ⚠️ 含 geo schema（见 4.1） | → `@a2ui/web-core` |
| a2ui-sdk | `packages/a2ui/sdk` | 20.6K 行 | ⚠️ 含 geo-catalog 资源 | → `@a2ui/sdk` |
| a2ui-agent | `packages/a2ui/agent` | 268 行 | ✅ LLM 生成器（DeepSeek/Ollama） | → `@a2ui/agent` |
...
├── packages/
│   ├── shared/                  # @a2ui/shared   （← geo-scout packages/a2ui/shared）
│   ├── web-core/                # @a2ui/web-core （← 同上，已 P0-1 净化）
│   ├── sdk/                     # @a2ui/sdk      （← 同上，已 P0-2 净化）
│   └── agent/                   # @a2ui/agent    （← 同上）
├── renderers/
│   └── angular/                 # @a2ui/angular  （← web a2ui 非 geo 部分）
│       ├── src/                 #   库代码（renderer 适配 + basic 组件 + catalog 注册）
│       ├── testing/             #   测试基座（echarts mock 等 geo 无关部分）
│       └── explorer/            #   演示壳（决策点 D7）
├── conformance/                 # @a2ui/conformance（← conformance-test）
├── eval/                        # @a2ui/eval     （← eval-test）
├── samples/
│   └── client/angular/          # 消费 @a2ui/angular 的最小演示（决策点 D7）
...
|---|---|---|
| `packages/a2ui/shared/src/**` | `packages/shared/src/**` | `@geo/a2ui-shared` → `@a2ui/shared` |
...
| `packages/a2ui/conformance-test/src/**` | `conformance/src/**` | 引用 `@a2ui/sdk` |
| `packages/a2ui/eval-test/src/**` | `eval/src/**` | 引用 `@a2ui/agent`/`@a2ui/sdk` |
| `apps/web/src/app/a2ui/renderer/**`（非 geo） | `renderers/angular/src/renderer/**` | `@geo/a2ui-web-core` → `@a2ui/web-core` |
...
| **geo 组件** `apps/web/src/app/a2ui/geo/**` | **留在 geo-scout**（决策点 D1） | 引用 `@a2ui/web-core`/`@a2ui/angular` 外部依赖 |

### 4.3 geo-scout 侧改动

| 文件 | 改动 |
|---|---|
| `package.json`/`pnpm-workspace.yaml` | 删除 a2ui 6 包；新增 `@a2ui/*` 外部依赖（registry 或 git） |
| `apps/web/src/app/a2ui/geo/**` + `geo-catalog.ts` | 保留；import 改外部包；geo schema 就地管理（P0-1 产物） |
| `apps/server/**` | 引擎/工具保留；`@geo/a2ui-sdk` → `@a2ui/sdk` |
...
| **D2** | 命名空间 | A. `@a2ui/*`（对齐官方，推荐）B. 保留 `@geo/a2ui-*` | **A** |
