...
> 现状：全部 a2ui 包以 `@geo/a2ui-*` scope 存在于 geo-scout；拆分后改名 `@a2ui/*` 并迁入独立仓库。

## 一、包名映射

| 现名（geo-scout）            | 新名（a2ui 项目）        | 目标目录                | 说明                                             |
| ---------------------------- | ------------------------ | ----------------------- | ------------------------------------------------ |
| `@geo/a2ui-shared`           | `@a2ui/shared`           | `packages/shared/`      | 零依赖；工具已下沉，P0-3 已反转 @geo/shared 依赖 |
| `@geo/a2ui-web-core`         | `@a2ui/web-core`         | `renderers/web_core/`   | P0-1 已移出 geo schema                           |
| `@geo/a2ui-sdk`              | `@a2ui/sdk`              | `packages/sdk/`         | P0-2 已移出 geo-catalog                          |
| `@geo/a2ui-agent`            | `@a2ui/agent`            | `packages/agent/`       | LLM 生成器（DeepSeek/Ollama）                    |
| `@geo/a2ui-conformance-test` | `@a2ui/conformance`      | `conformance/`          | v1.0 一致性测试套件                              |
| `@geo/a2ui-eval-test`        | `@a2ui/eval`             | `eval/`                 | LLM 评估框架                                     |
...
| 包                     | 现依赖                                   | 改后依赖                                         |
| ---------------------- | ---------------------------------------- | ------------------------------------------------ |
| `apps/server`          | `@geo/a2ui-sdk`、`@geo/a2ui-shared`      | `@a2ui/sdk`、`@a2ui/shared`（外部 registry/git） |
| `apps/web`             | `@geo/a2ui-shared`、`@geo/a2ui-web-core` | `@a2ui/shared`、`@a2ui/web-core`                 |
| `packages/geo-catalog` | （无 a2ui 依赖）                         | 不变                                             |
| a2ui 包内部            | `@geo/a2ui-*` workspace 互依             | `@a2ui/*` workspace 互依                         |
...
1. **字符串替换**（全仓，含 package.json 与 import）：
   - `@geo/a2ui-shared` → `@a2ui/shared`
   - `@geo/a2ui-web-core` → `@a2ui/web-core`
   - `@geo/a2ui-sdk` → `@a2ui/sdk`
   - `@geo/a2ui-agent` → `@a2ui/agent`
   - `@geo/a2ui-conformance-test` → `@a2ui/conformance`
   - `@geo/a2ui-eval-test` → `@a2ui/eval`
...
- 工具函数（normalizeSmartQuotes 等）geo-scout 通过 `@a2ui/shared` 消费（P0-3 已就绪）
