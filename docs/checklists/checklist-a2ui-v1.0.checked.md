...
| 7.9 | 渲染端校验失败时，error `code` 是否使用规范枚举（`VALIDATION_FAILED` / `UNALLOWED_PARENT` / `UNALLOWED_CHILD`，后两者上游 #2155 新增）？ | **✅（2026-08-25 补齐）：web-core 新增 `composition-constraints.ts`（约束注册表 + `checkCompositionConstraints`），message-handler 组件校验链路接入后主动产生 UNALLOWED_PARENT/UNALLOWED_CHILD 标准错误码（path 为违规组件 ID），SDK schema 接受三枚举** |
...
| 10.2 | 函数 Schema 是否设置了 `allowedCallers`，且枚举值严格为 `rendererOnly`、`agentOnly`、`rendererOrAgent` 之一（默认 `rendererOnly`）？ | **❌ 无虚拟滚动（1000+ 模板实例全量渲染）** |
...
| 11.9 | `callRendererFunction` 的 `allowedCallers` 组合是否正确执行（`rendererOnly` 拒绝、`agentOnly` 允许、`rendererOrAgent` 允许）？（v1.0 #2210） | ✅  |
